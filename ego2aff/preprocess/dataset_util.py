...
def load_ho_annot_ego4d(video_detections, frame_index, imgW, imgH):
    annot = video_detections[frame_index] # frame_index start from 1
    assert annot.frame_number == frame_index, "wrong frame index"
    annot.scale(width_factor=imgW, height_factor=imgH)
    return annot
...
def load_img_ego4d(clip, frame_index):
    clip.set(cv2.CAP_PROP_POS_FRAMES, frame_index)
    res, frame = clip.read()
    if res:
        return frame
    else:
        ValueError('read frame failed')
    return frame
...
def fetch_data(frames_path, video_detections, frames_idxs, hand_threshold=0.1, obj_threshold=0.1):
    ...
    # 检测video clip有没有检测到手（除了最后start frame）
...
def fetch_data_ego4d(clip, video_detections, frames_idxs, frame_size, hand_threshold=0.1, obj_threshold=0.1, tolerance=5):
    # tolerance = frames_idxs[1] - frames_idxs[0] # extend future act frame by tolerance to find ho interaction
    frames = []
    annots = []
    W, H = frame_size
    ...
    miss_hand = 0
    for frame_idx in frames_idxs[:-1]:
        frame = load_img_ego4d(clip, frame_idx)
        annot = load_ho_annot_ego4d(video_detections, frame_idx, W, H)
        hands = [hand for hand in annot.hands if hand.score >= hand_threshold]
        if len(hands) == 0:
            miss_hand += 1
        frames.append(frame)
    ...
    if flag:
        frames_idxs.append(frame_idx)
        frames.append(load_img_ego4d(clip, frame_idx))
        annots.append(annot)
        return frames_idxs, frames, annots, list(set(hand_sides)) # remove redundant hand sides
    else:
        return None

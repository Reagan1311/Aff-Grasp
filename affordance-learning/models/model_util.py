...
def softdice_loss(pred, gt, smooth=1):
    # pred, gt: B x N x H x W
    pred = pred.flatten(1)
    gt = gt.flatten(1)
    numer = 2 * (pred * gt).sum(-1)
    denor = (pred + gt).sum(-1)
    loss = 1. - (numer + smooth) / (denor + smooth)
    return loss.mean()


def sigmoid_focal_loss(pred, gt, alpha: float = 0.25, gamma: float = 2):
...
def multi_concentration_loss(pred):
    # b x h x w
    B = pred.shape[0]
    total_loss = 0
    for p in pred:
        C, H, W = p.shape
        tmp_max, tmp_min = p.max(-1)[0].max(-1)[0].view(C, 1, 1), \
                           p.min(-1)[0].min(-1)[0].view(C, 1, 1)

        pred_norm = ((p - tmp_min) / (tmp_max - tmp_min + 1e-10))  # b x 28 x 28

        loss = 0
        epsilon = 1e-3
        centers_all = batch_get_centers(pred_norm)
        for c in range(C):
            centers = centers_all[c]
            # normalize part map as spatial pdf
            part_map = pred_norm[c, :, :] + epsilon  # prevent gradient explosion
            k = part_map.sum()
            part_map_pdf = part_map / k
            x_c, y_c = centers

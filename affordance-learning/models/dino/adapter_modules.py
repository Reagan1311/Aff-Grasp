...
class InjectorBlock(nn.Module):
    def __init__(self, dim, num_reg=4, num_heads=6, n_points=4, norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 init_values=0., deform_ratio=1.0, with_cp=False):
        super().__init__()
        self.injector = Injector(dim=dim, n_levels=1, num_heads=num_heads, init_values=init_values,
                                 n_points=n_points, norm_layer=norm_layer, deform_ratio=deform_ratio,
                                 with_cp=with_cp)
        self.num_reg = num_reg

    def forward(self, x, c, cls, blocks, deform_inputs1):
        ...
        for idx, blk in enumerate(blocks):
            x = blk(x)
        cls, x = x[:, :self.num_reg + 1, ], x[:, self.num_reg + 1:, ]
        return x, cls
...
class DepthSPM(nn.Module):
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            bs, dim, _, _ = c3.shape
            c3_up = F.interpolate(c3, size=(h//14, w//14), mode='bilinear', align_corners=False)
            c3 = c3_up.view(bs, dim, -1).transpose(1, 2)
            # c1 = c1.view(bs, dim, -1).transpose(1, 2)  # 4s
            # c2 = c2.view(bs, dim, -1).transpose(1, 2)  # 8s
            # c3 = c3.view(bs, dim, -1).transpose(1, 2)  # 16s
            # c4 = c4.view(bs, dim, -1).transpose(1, 2)  # 32s
...
class RSSPM(nn.Module):
    def __init__(self, inplanes=64, embed_dim=384, with_cp=False):
        super().__init__()
        self.with_cp = with_cp

        self.stem = nn.Sequential(*[
            nn.Conv2d(10, inplanes, kernel_size=3, stride=2, padding=1, bias=False),
            nn.SyncBatchNorm(inplanes),
            nn.ReLU(inplace=True),
            nn.Conv2d(inplanes, inplanes, kernel_size=3, stride=1, padding=1, bias=False),
            nn.SyncBatchNorm(inplanes),
    ...
    def forward(self, x):
        ...
        def _inner_forward(x):
            ...
            # c2 = F.interpolate(c2, size=(h // 8, w // 8), mode='bilinear', align_corners=False)
            c2 = c2.view(bs, dim, -1).transpose(1, 2)  # 8s
            # c3 = F.interpolate(c3, size=(h // 16, w // 16), mode='bilinear', align_corners=False)
            c3 = c3.view(bs, dim, -1).transpose(1, 2)  # 16s
            # c4 = F.interpolate(c4, size=(h // 32, w // 32), mode='bilinear', align_corners=False)
            c4 = c4.view(bs, dim, -1).transpose(1, 2)  # 32s
